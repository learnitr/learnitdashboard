...
#'
#' @param data_table The data frame from which the url and alt_url are prepared.
#'
#' @return The given data frame with the url and alt_url prepared as html link.
#' @export
#'
#' @examples
...
prepare_url_alr_url <- function(data_table) {
  ...
  if (!any(!c("url", "alt_url","template") %in% names(data_table))) {
    ...
    data_table$url <- paste0('<a href="', data_table$url,
      '" target="_blank">', data_table$url, '</a>')
    # Preparing the alt_url in html link
    data_table$alt_url <- paste0('<a href="', data_table$alt_url,
      '" target="_blank">',data_table$alt_url, '</a>')
    # Preparing the template in html link
    data_table$template <- paste0('<a href="', data_table$template,
      '" target="_blank">', data_table$template, '</a>')
  ...
  # If only url and avatar
    ...
    data_table$url <- paste0('<a href="', data_table$url,
      '" target="_blank">', data_table$url, '</a>')
    # Preparing the alt_url in html link
    data_table$avatar <- paste0('<a href="', data_table$avatar,
      '" target="_blank">', data_table$avatar, '</a>')
